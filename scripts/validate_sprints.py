...
def mock_ask_llm_structured(prompt: str, response_schema, **kwargs) -> Any:
    if response_schema == AgentAction:
        ...
        for name in ["python_cli", "fastapi_api", "nextjs_ts", "react_web", "hospital_sys", "ecommerce_sys"]:
            if name in p:
                project = name
...
def run_validation():
    ...
    projects = [
        {"name": "python_cli", "bug": "missing_import", "desc": "Python CLI Project (missing import)"},
        {"name": "fastapi_api", "bug": "import_bug", "desc": "FastAPI Project (import error)"},
        {"name": "nextjs_ts", "bug": "typescript_bug", "desc": "Next.js + TypeScript Project (type mismatch)"},
        {"name": "react_web", "bug": "unclosed_tag", "desc": "React Project (unclosed JSX tag)"},
        {"name": "hospital_sys", "bug": "broken_import", "desc": "Hospital Management System (broken models import)"},
        {"name": "ecommerce_sys", "bug": "index_error", "desc": "E-commerce System (Runtime IndexError)"}
    ]
    ...
    for proj in projects:
        ...
        res = engine.run_consensus(task_id, f"Resolve injected bug '{proj['bug']}' in {proj['name']}")
...
if __name__ == "__main__":
    with patch("core.llm._global_wrapper.generate", side_effect=mock_generate):
        with patch("core.llm._global_wrapper.generate_structured", side_effect=mock_generate_structured):
            run_validation()
