...
from core.database import init_db, get_db_session, Task, TaskLog, AgentMessage, MemoryEntry, WorkflowExecution
            ...
            if tasks_dir.exists():
                ...
                for task_file in tasks_dir.glob("*.json"):
                    try:
                        ...
                        if not task_id:
                            print(f"   [Warning] Skipped invalid task file: {task_file}")
                            continue
                            
                        # Format timestamps
                        created_at = datetime.fromisoformat(data.get("created_at")) if data.get("created_at") else datetime.now(timezone.utc).replace(tzinfo=None)
                        ...
                        print(f"   [Error] Failed to read/parse task file {task_file}: {str(te)}")
                        raise te
            ...
            for session_file in persist_dir.glob("*.json"):
                ...
                try:
                    ...
                    for log_entry in logs_list:
                        ts = datetime.fromisoformat(log_entry.get("timestamp")) if log_entry.get("timestamp") else datetime.now(timezone.utc).replace(tzinfo=None)
                        db_log = TaskLog(
                            task_id=session_id,
                    ...
                    for msg_entry in msgs_list:
                        ts = datetime.fromisoformat(msg_entry.get("timestamp")) if msg_entry.get("timestamp") else datetime.now(timezone.utc).replace(tzinfo=None)
                        db_msg = AgentMessage(
                            task_id=session_id,
                    ...
                    for step_entry in steps_list:
                        start_t = datetime.fromisoformat(step_entry.get("started_at")) if step_entry.get("started_at") else None
                        end_t = datetime.fromisoformat(step_entry.get("completed_at")) if step_entry.get("completed_at") else None
                        db_step = WorkflowExecution(
                            ...
                            assigned_agent=step_entry.get("assigned_agent", "developer"),
                    ...
                    print(f"   [Error] Failed to read/parse memory session file {session_file}: {str(se)}")
                    raise se
                ...
                for index_file in memory_dir.glob("*.json"):
                    try:
                        ...
                        for item in items_data:
                            ts = datetime.fromisoformat(item.get("timestamp")) if item.get("timestamp") else datetime.now(timezone.utc).replace(tzinfo=None)
                        ...
                        print(f"   [Error] Failed to read/parse vector index file {index_file}: {str(me)}")
                        raise me
