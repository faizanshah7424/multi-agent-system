...
class TaskResponse(BaseModel):
    ...
    session_id: str = Field(
        ..., description="The unique session ID associated with the task."
    )
    status: str = Field(..., description="The current status of the task workflow.")
    message: str = Field(
        ..., description="Informational message about the workflow initialization."
    )
...
class TaskCreateRequest(BaseModel):
    ...
    task_id: Optional[str] = Field(
        default=None, max_length=100, description="Optional custom task identifier."
    )
    user_id: Optional[str] = Field(
        default=None, max_length=100, description="Optional user ID owner."
    )
...
class ChatRequest(BaseModel):
    ...
    message: str = Field(
        ...
        min_length=1,
        max_length=50000,
        description="The text query to send to the agent.",
    )
    session_id: Optional[str] = Field(
        default=None,
        max_length=100,
...
class StatusResponse(BaseModel):
    ...
    status: str = Field(
        ..., description="Overall workflow status (idle, running, completed, failed)."
    )
