...
def post_task(request: TaskRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to initialize memory session."
        )

    # Queue task to execute asynchronously via the TaskQueue
    try:
        task_manager.create_task(
            task_id=session_id, payload={"task": request.task}, user_id=None
        )
    except Exception as e:
...
def post_chat(request: ChatRequest):
    ...
    try:
        ...
        return ChatResponse(session_id=session_id, response=reply_content)
...
def get_status(
    ...
    user_id_query: Optional[str] = Query(
        None, alias="user_id", description="Optional user ID for ownership validation."
    ),
):
    """
    ...
    try:
        ...
        return StatusResponse(
            session_id=session_id, status=memory.state.status, steps=steps
        )
    except Exception as e:
...
def get_history(
    ...
    try:
        ...
        return HistoryResponse(session_id=session_id, logs=logs, messages=messages)
    except Exception as e:
...
def create_task(request: TaskCreateRequest):
    ...
    try:
        ...
        try:
            ...
            task_prompt = request.payload.get(
                "task", f"Custom task execution: {request.task_type}"
            )
            memory.add_log(
                "system", f"Queuing background task execution: '{task_prompt}'"
            )
        except Exception as e:
            logger.warning(
                f"Failed to initialize memory tracking for task {task_id}: {str(e)}"
            )

        task = task_manager.create_task(
            task_id=task_id, payload=request.payload, user_id=request.user_id
        )
        return task
    except Exception as e:
        logger.error(f"Failed to create task: {str(e)}")
        raise HTTPException(
            status_code=500, detail=f"Failed to create and queue task: {str(e)}"
        )
    ...
    try:
        with get_db_session() as session:
            ...
            for w in db_workers:
                uptime = (
                    (
                        datetime.now(timezone.utc).replace(tzinfo=None) - w.startup_time
                    ).total_seconds()
                    if w.startup_time
                    else 0.0
                )
...
def search_memory(
    query: str = Query(
        ...
        min_length=1,
        max_length=500,
        description="The natural language query to run similarity search on.",
    ),
    limit: int = Query(
        default=3, ge=1, le=50, description="Maximum number of memory items to return."
    ),
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to query semantic vector memory: {str(e)}"
        )
...
def consolidate_memory(
    session_id: str = Query(
        ...,
        pattern=r"^[a-zA-Z0-9_\-]+$",
        description="The memory session ID to consolidate into long-term vector index.",
    ),
    user_id: Optional[str] = Header(None, alias="X-User-ID"),
    user_id_query: Optional[str] = Query(None, alias="user_id"),
):
    ...
    try:
        ...
        if not result:
            raise HTTPException(
                status_code=404,
                detail=f"Session ID '{session_id}' not found or has empty logs.",
            )
        return {
            "status": "success",
            "session_id": session_id,
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to consolidate memory session: {str(e)}"
        )
...
def trigger_autonomous_execution(
    ...
    target_files: str = Query(
        ..., description="Comma-separated files impacted by the goal."
    ),
):
    """
...
def list_engineering_records():
    ...
    """
    from core.autonomous_engineering.engineering_engine import (
...
def trigger_autonomous_fix(
    stack_trace: str = Query(
        ..., description="The crash stack trace to analyze and resolve."
    ),
    failing_tests: Optional[str] = Query(
        None, description="Comma-separated list of failing tests."
    ),
):
    """
    ...
    engine = AutonomousEngineeringEngine()
    tests_list = (
        [t.strip() for t in failing_tests.split(",") if t.strip()]
        if failing_tests
        else None
    )
...
def trigger_feature_development(
    requirement: str = Query(
        ..., description="The natural language feature requirement."
    )
):
    """
