...
def refresh_token(request: RefreshRequest):
    ...
    with get_db_session() as session:
        record = session.query(RefreshTokenRecord).filter(
...
def test_dev_or_admin_route(current_user: dict = Depends(RoleChecker(["admin", "developer"]))):
    return {
        "message": "Authorized for developer/admin access.",
        "user": current_user
    }
