...
def update_user(
    ...
    with get_db_session() as session:
        ...
        if payload.role is not None and payload.role != user.role:
            role_exists = (
                session.query(RoleRecord)
                .filter(RoleRecord.name == payload.role)
                .first()
                is not None
            )
            if not role_exists:
                raise HTTPException(
                    status_code=400, detail=f"Role '{payload.role}' does not exist."
                )
        ...
        if payload.is_active is not None and payload.is_active != user.is_active:
            changes.append(
                f"is_active from '{user.is_active}' to '{payload.is_active}'"
            )
...
def get_roles(current_user: dict = Depends(PermissionChecker("roles:read"))):
    with get_db_session() as session:
        ...
        for r in roles:
            perms = (
                session.query(PermissionRecord)
                .join(
                    RolePermissionRecord,
                    RolePermissionRecord.permission_id == PermissionRecord.id,
                )
                .filter(RolePermissionRecord.role_id == r.id)
                .all()
            )
...
def update_role_permissions(
    ...
    with get_db_session() as session:
        ...
        db_perms = (
            session.query(PermissionRecord)
            .filter(PermissionRecord.name.in_(payload.permission_names))
            .all()
        )
        if len(db_perms) != len(payload.permission_names):
            raise HTTPException(
                status_code=400, detail="One or more permissions are invalid."
            )

        session.query(RolePermissionRecord).filter(
...
def get_audit_logs(
    ...
    with get_db_session() as session:
        logs = (
            session.query(AuthAuditLogRecord)
