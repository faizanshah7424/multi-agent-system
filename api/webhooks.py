...
async def clerk_webhook(
    ...
    if not secret:
        ...
        if settings.authentication_backend != "mock":
            logger.error(
                "CLERK_WEBHOOK_SECRET environment variable is missing in production. Rejecting Clerk webhook."
            )
            raise HTTPException(status_code=500, detail="Webhook configuration error")
        logger.warning(
            "CLERK_WEBHOOK_SECRET is missing. Bypassing signature verification because AUTHENTICATION_BACKEND is 'mock'."
        )
        ...
        # Role / Membership synchronization
            ...
            if not membership:
                membership = OrganizationMembership(
                    id=membership_id, user_id=user_id, org_id=org_id, role=role
                )
            ...
            session.commit()
            logger.info(
                f"Synced Membership {membership_id} (User: {user_id}, Org: {org_id}, Role: {role})"
            )
...
async def stripe_webhook(
    ...
    if not webhook_secret and not is_mock:
        logger.error(
            "STRIPE_WEBHOOK_SECRET environment variable is missing in production. Rejecting Stripe webhook."
        )
        raise HTTPException(status_code=500, detail="Webhook configuration error")
    ...
    try:
        if webhook_secret and stripe_signature:
            ...
            logger.error(
                "Stripe signature missing in production. Rejecting Stripe webhook."
            )
    ...
    event_data = (
        ...
        else getattr(getattr(event, "data", {}), "object", {})
    )
    ...
    if event_type == "checkout.session.completed":
        org_id = event_data.get("client_reference_id") or event_data.get(
            "metadata", {}
        ).get("org_id")
        ...
        if org_id and sub_id:
            ...
            logger.info(
                f"Successfully processed checkout for Org {org_id} (Subscription: {sub_id})"
            )
    ...
    # 2. Handle subscription modifications (seat scaling, plan changes)
        ...
        # Format timestamps
        dt_trial_start = (
            datetime.fromtimestamp(trial_start, tz=timezone.utc).replace(tzinfo=None)
            if trial_start
            else None
        )
        dt_trial_end = (
            datetime.fromtimestamp(trial_end, tz=timezone.utc).replace(tzinfo=None)
            if trial_end
            else None
        )
        dt_period_end = (
    ...
    # 3. Handle subscription cancellations / delinquencies
        ...
        with get_db_session() as session:
            ...
            if sub:
                ...
                session.commit()
                logger.info(
                    f"Canceled subscription {sub_id} and downgraded Org {sub.org_id} to Free."
                )
