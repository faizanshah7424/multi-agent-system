...
def create_patient(
    payload: PatientCreateSchema, current_user: dict = Depends(get_current_user)
):
    with get_db_session() as session:
...
@router.post("/doctors")
def create_doctor(
    payload: DoctorCreateSchema, current_user: dict = Depends(get_current_user)
):
    with get_db_session() as session:
        doctor = DoctorRecord(
            name=payload.name,
    ...
    with get_db_session() as session:
        ...
        for appt in appointments:
            patient = (
                ...
                .filter(PatientRecord.id == appt.patient_id)
                .first()
            )
            doctor = (
                session.query(DoctorRecord)
                .filter(DoctorRecord.id == appt.doctor_id)
                .first()
            )
            result.append(
                {
...
def create_appointment(
    payload: AppointmentCreateSchema, current_user: dict = Depends(get_current_user)
):
    with get_db_session() as session:
        # Validate patient and doctor exist
        p = (
            session.query(PatientRecord)
            .filter(PatientRecord.id == payload.patient_id)
            .first()
        )
        d = (
            session.query(DoctorRecord)
            .filter(DoctorRecord.id == payload.doctor_id)
            .first()
        )
        if not p or not d:
...
def update_appointment_status(
    appointment_id: str, status: str, current_user: dict = Depends(get_current_user)
):
    ...
    with get_db_session() as session:
        appt = (
            ...
            .filter(AppointmentRecord.id == appointment_id)
            .first()
        )
        if not appt:
...
def get_billing(current_user: dict = Depends(get_current_user)):
    with get_db_session() as session:
        ...
        for b in bills:
            patient = (
                session.query(PatientRecord)
                .filter(PatientRecord.id == b.patient_id)
                .first()
            )
            result.append(
                {
...
@router.post("/billing")
def create_billing(
    payload: BillingCreateSchema, current_user: dict = Depends(get_current_user)
):
    with get_db_session() as session:
        # Validate patient
        p = (
...
def pay_billing(
    billing_id: str,
    payload: PayBillSchema,
    current_user: dict = Depends(get_current_user),
):
    with get_db_session() as session:
        bill = (
            session.query(BillingRecord).filter(BillingRecord.id == billing_id).first()
        )
        if not bill:
...
def create_pharmacy_item(
    payload: PharmacyItemCreateSchema, current_user: dict = Depends(get_current_user)
):
    with get_db_session() as session:
        # Check duplicate name
        existing = (
            session.query(PharmacyItemRecord)
            .filter(PharmacyItemRecord.name == payload.name)
            .first()
        )
        if existing:
...
def get_prescriptions(current_user: dict = Depends(get_current_user)):
    with get_db_session() as session:
        ...
        for pr in prescriptions:
            patient = (
                session.query(PatientRecord)
                .filter(PatientRecord.id == pr.patient_id)
                .first()
            )
            doctor = (
                session.query(DoctorRecord)
                .filter(DoctorRecord.id == pr.doctor_id)
                .first()
            )
            result.append(
                {
...
def create_prescription(
    payload: PrescriptionCreateSchema, current_user: dict = Depends(get_current_user)
):
    with get_db_session() as session:
        p = (
            session.query(PatientRecord)
...
def dispense_prescription(
    ...
    with get_db_session() as session:
        ...
        if pr.status == "dispensed":
            raise HTTPException(
                status_code=400, detail="Prescription already dispensed."
            )

        # Deduct stock if item exists in inventory
        item = (
            session.query(PharmacyItemRecord)
            .filter(PharmacyItemRecord.name == pr.medication_name)
            .first()
        )
        if item:
            if item.stock_quantity > 0:
                ...
                raise HTTPException(
                    status_code=400, detail="Out of stock for prescribed medication."
                )
...
def get_lab_tests(current_user: dict = Depends(get_current_user)):
    with get_db_session() as session:
        ...
        for t in tests:
            patient = (
                session.query(PatientRecord)
                .filter(PatientRecord.id == t.patient_id)
                .first()
            )
            result.append(
                {
...
def create_lab_test(
    payload: LabTestCreateSchema, current_user: dict = Depends(get_current_user)
):
    with get_db_session() as session:
        p = (
            session.query(PatientRecord)
        ...
        test = LabTestRecord(
            patient_id=payload.patient_id, test_name=payload.test_name, status="pending"
        )
        session.add(test)
        session.commit()
...
def get_hospital_stats(current_user: dict = Depends(get_current_user)):
    with get_db_session() as session:
        ...
        scheduled_appts = (
            session.query(AppointmentRecord)
            .filter(AppointmentRecord.status == "scheduled")
            .count()
        )
        ...
        pending_prescriptions = (
            session.query(PrescriptionRecord)
            .filter(PrescriptionRecord.status == "prescribed")
            .count()
        )
        ...
        pending_tests = (
            session.query(LabTestRecord)
            .filter(LabTestRecord.status == "pending")
            .count()
        )
        ...
        return {
            ...
            "appointments": {"total": total_appointments, "scheduled": scheduled_appts},
            "billing": {
            ...
            "laboratory": {"total_tests": total_tests, "pending": pending_tests},
        }
