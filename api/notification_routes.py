...
def update_preferences_endpoint(
    payload: PreferencesUpdateSchema, current_user: dict = Depends(get_current_user)
):
    """Update notification preferences for the user."""
    with get_db_session() as session:
        pref = update_preferences(
            current_user["id"], payload.model_dump(exclude_none=True), session
        )
        return {
...
def get_notification_stats(
    ...
    with get_db_session() as session:
        ...
        unread_in_app = (
            session.query(NotificationRecord)
            .filter(NotificationRecord.is_read.is_(False))
            .count()
        )
        ...
        pending_queued = (
            ...
            .filter(NotificationQueueRecord.status == "pending")
            .count()
        )
        sent_queued = (
            session.query(NotificationQueueRecord)
            .filter(NotificationQueueRecord.status == "sent")
            .count()
        )
        failed_queued = (
            session.query(NotificationQueueRecord)
            .filter(NotificationQueueRecord.status == "failed")
            .count()
        )
        retrying_queued = (
            session.query(NotificationQueueRecord)
            .filter(NotificationQueueRecord.status == "retrying")
            .count()
        )
        ...
        email_queued = (
            session.query(NotificationQueueRecord)
            .filter(NotificationQueueRecord.channel == "email")
            .count()
        )
        sms_queued = (
            session.query(NotificationQueueRecord)
            .filter(NotificationQueueRecord.channel == "sms")
            .count()
        )
        ...
        recent_items = (
            session.query(NotificationQueueRecord)
            .order_by(NotificationQueueRecord.created_at.desc())
            .limit(10)
            .all()
        )
        recent_list = [
        ...
        return {
            "in_app": {"total": total_in_app, "unread": unread_in_app},
            "queue": {
                "total": total_queued,
            ...
            "channels": {"email": email_queued, "sms": sms_queued},
...
def trigger_test_notification(
    payload: TestNotificationSchema, current_user: dict = Depends(get_current_user)
):
...
def trigger_process_queue(
    current_user: dict = Depends(PermissionChecker("tasks:create")),
):
