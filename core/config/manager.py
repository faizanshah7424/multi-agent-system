...
class SystemConfig(BaseModel):
    ...
    env: str = Field("development", description="Environment: development, production, enterprise")
    gemini_api_key: str = Field("", description="Google Gemini API credential key")
    openai_api_key: str = Field("", description="OpenAI API key")
    anthropic_api_key: str = Field("", description="Anthropic API key")
    database_url: str = Field("sqlite:///data/system.db", description="Database connection URL")
    api_host: str = Field("127.0.0.1", description="FastAPI host address")
    api_port: int = Field(8000, description="FastAPI port")
    persist_dir: str = Field("data", description="Directory path for local files persistence")
...
class ConfigurationManager:
    ...
    def validate(self) -> Dict[str, Any]:
        ...
        if not (1024 <= self.config.api_port <= 65535):
            errors.append(f"Invalid API_PORT: {self.config.api_port}. Must be between 1024 and 65535.")

        # 2. Database Dialect check
        if not (self.config.database_url.startswith("sqlite") or self.config.database_url.startswith("postgresql")):
            errors.append(f"Unsupported DATABASE_URL dialect. Supported: sqlite, postgresql.")

        # 3. Environment credentials check
        if self.config.env == "production" and not self.config.gemini_api_key:
            errors.append("Production environment requires GEMINI_API_KEY to be configured.")
