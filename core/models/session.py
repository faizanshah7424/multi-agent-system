...
class Message(BaseModel):
    ...
    tool_calls: Optional[List[ToolCall]] = Field(
        None, description="Optional tool calls."
    )
    tool_results: Optional[List[ToolResult]] = Field(
        None, description="Optional tool execution results."
    )
...
class Conversation(BaseModel):
    ...
    total_usage: Usage = Field(
        default_factory=Usage, description="Aggregate token usage."
    )
    total_cost: Cost = Field(
        default_factory=Cost, description="Aggregate financial cost."
    )
