...
class FeatureBuilder:
    ...
    """
    def build_feature(self, spec: FeatureSpec, plan: FeatureExecutionPlan) -> Dict[str, Any]:
        generated_artifacts = {
            "backend": "",
            "frontend": "",
            "database": "",
        ...
        if "inventory" in goals_str:
            ...
            generated_artifacts["api"] = "@router.post('/api/custom')\ndef custom_api():\n    return {'status': 'ok'}\n"
            generated_artifacts["database"] = "-- No database changes needed --"
            generated_artifacts["frontend"] = "export const CustomView = () => <div>Custom Feature View</div>;"
            generated_artifacts["tests"] = "def test_custom_feature():\n    assert True\n"

        generated_artifacts["documentation"] = (
            f"# Feature: {spec.goals[0] if spec.goals else 'Custom'}\n\n"
            "## Goals\n" + "\n".join([f"- {g}" for g in spec.goals])
        )
        generated_artifacts["validation"] = "def validate_feature_integrity():\n    return {'status': 'passed'}\n"
