...
class ApiDesigner:
    ...
    def design_apis(self, model: DomainModel) -> ApiDesignSpecs:
        endpoints = [
            EndpointSpec(
                method="POST", path="/auth/login", description="Authenticate sessions"
            ),
            EndpointSpec(
                method="POST", path="/auth/signup", description="Enroll users"
            ),
        ]
        ...
        analytics = [
            EndpointSpec(
                ...
                description="Fetch business KPI summaries",
            ),
            EndpointSpec(
                method="GET",
                path="/api/analytics/trends",
                description="Fetch billing and admissions rate charts data",
            ),
        ]
