    ...
    def analyze_business_idea(self, idea: str) -> BusinessSpecs:
        ...
        if "hospital" in idea_lower:
            return BusinessSpecs(
                ...
                permissions=["read_patient_record", "write_prescription", "manage_billing"],
                workflows=["Patient Check-in", "Doctor consultation & Prescription", "Discharge & Billing"],
                entities=["Patient", "Doctor", "Appointment", "Prescription", "Invoice"],
                reports=["Daily Admissions", "Billing Summary", "Bed Occupancy Rates"],
                kpis=["Average Wait Time", "Patient Satisfaction Index", "Bed Turnover Rate"]
            )
        elif "restaurant" in idea_lower or "pos" in idea_lower:
            return BusinessSpecs(
                domain="Food & Beverage / Retail Point of Sale",
                stakeholders=["Customers", "Waitstaff", "Kitchen Chef", "Store Manager"],
                user_roles=["Cashier", "Waiter", "Chef", "Manager"],
                permissions=["place_order", "process_payment", "modify_menu", "refund_transaction"],
                workflows=["Order creation", "Kitchen notification", "Payment checkout"],
                entities=["Order", "MenuProduct", "Payment", "TableRecord"],
                reports=["Hourly Sales Volume", "Top Selling Items", "Staff Tips Log"],
                kpis=["Average Ticket Size", "Order Cycle Time", "Inventory Waste Rate"]
            )
        else:
            return BusinessSpecs(
