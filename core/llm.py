...
if not api_key:
    logger.warning(
        "GEMINI_API_KEY is not set in environment or settings. API calls may fail."
    )
...
class GeminiWrapper:
    ...
    def generate(
        ...
        schema_str = (
            ...
            else str(response_schema) if response_schema else None
        )
        ...
        try:
            logger.debug(
                f"Calling model '{target_model}' (JSON/Schema: {json_mode or (response_schema is not None)})..."
            )
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if attempt < retries:
                    ...
                    logger.error(
                        f"Structured output generation permanently failed after {retries+1} attempts."
                    )
                    raise e
