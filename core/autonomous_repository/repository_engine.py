...
from core.autonomous_repository.repository_context import RepositoryContextDetector, RepositoryContext
from core.autonomous_repository.repository_planner import RepositoryPlanner, RepositoryTaskPlan
...
class AutonomousRepositoryEngine:
    ...
    def run_repository_engineering(self, goal: str, apply_changes_fn: Optional[Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            db_script = "\n".join([f"-- migration: {change}" for change in plan.database_changes])
            ...
            if not val_res["success"]:
                raise RuntimeError(f"Repository validation failed: {val_res['results']}")

            # 9. Documentation Reports Generation
            reports_paths = self.report_gen.generate_all_reports(goal, plan.model_dump(), val_res)
