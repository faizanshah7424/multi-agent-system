...
class FileChange(BaseModel):
    ...
    action: Literal["add", "modify", "delete"] = Field(..., description="Nature of the filesystem change.")
    content: str = Field(..., description="Full content (for additions/modifications) or unified diff.")
...
class SessionState(BaseModel):
    ...
    workspace_path: str = Field(..., description="Absolute path to the isolated host worktree directory.")
    container_id: Optional[str] = Field(None, description="Docker container identifier if active.")
    git_branch: str = Field(..., description="Isolated workspace git branch name.")
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Time of session creation.")
    last_active: datetime = Field(default_factory=datetime.utcnow, description="Time of last heartbeat/action.")
