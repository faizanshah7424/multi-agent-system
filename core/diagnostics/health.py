...
class RepositoryHealthInspector:
    ...
    def run_diagnostics(self) -> HealthReport:
        ...
        try:
            ...
            items.append(
                HealthCheckItem(
                    ...
                    status=True,
                    details="Workspace path exists and is writable.",
                )
            )
            ...
            items.append(
                HealthCheckItem(
                    name="Workspace Integrity",
                    status=False,
                    details=f"Workspace path is unwritable: {e}",
                )
            )
        ...
        try:
            ...
            items.append(
                HealthCheckItem(
                    name="Disk Usage", status=False, details=f"Disk check failed: {e}"
                )
            )
        ...
        try:
            ...
            if res.returncode == 0:
                ...
                if uncommitted:
                    git_clean = False
                    details = (
                        f"Uncommitted local changes detected:\n{uncommitted[:200]}"
                    )
                else:
                    details = "Git repository is clean."
                items.append(
                    HealthCheckItem(
                        name="Git Cleanliness", status=True, details=details
                    )
                )
            else:
                items.append(
                    HealthCheckItem(
                        ...
                        status=False,
                        details="Not a valid git repository or git error.",
                    )
                )
        except Exception as e:
            items.append(
                HealthCheckItem(
                    name="Git Cleanliness",
                    status=False,
                    details=f"Git binary not found or failed: {e}",
                )
            )
        ...
        try:
            ...
            items.append(
                HealthCheckItem(
                    ...
                    status=True,
                    details="Successfully executed query on database session.",
                )
            )
        except Exception as e:
            overall = False
            items.append(
                HealthCheckItem(
                    name="Database Reachability",
                    status=False,
                    details=f"Database connect failed: {e}",
                )
            )
        ...
        try:
            ...
            details = (
                "Docker Daemon is online and accessible."
                if status
                else "Docker Daemon returned error exit code."
            )
            items.append(
                HealthCheckItem(name="Docker Status", status=status, details=details)
            )
        except Exception as e:
            ...
            items.append(
                HealthCheckItem(
                    name="Docker Status",
                    status=False,
                    details=f"Docker CLI not found or offline: {e}",
                )
            )
        ...
        try:
            ...
            details = (
                "Primary Gemini API Key configured."
                if has_primary
                else "Missing GEMINI_API_KEY."
            )
            items.append(
                HealthCheckItem(
                    name="Model Provider API", status=has_primary, details=details
                )
            )
        except Exception as e:
            items.append(
                HealthCheckItem(
                    ...
                    status=False,
                    details=f"Secret manager not initialized: {e}",
                )
            )
