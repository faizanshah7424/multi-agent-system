...
class InjectableBug(BaseModel):
    ...
    category: str = Field(..., description="Failure category (e.g. SYNTAX_ERROR, IMPORT_ERROR, TYPE_ERROR, LINT_ERROR, UNIT_TEST_FAILURE)")
...
class BenchmarkProject(BaseModel):
    ...
    category: str = Field(..., description="Platform/Technology category (Python, FastAPI, TypeScript, React, Next.js, etc.)")
    files: Dict[str, str] = Field(..., description="Mapping of relative paths to file contents.")
    injectable_bugs: List[InjectableBug] = Field(default_factory=list, description="List of supportable bugs.")
...
class BenchmarkReport(BaseModel):
    ...
    scores: Dict[str, float] = Field(default_factory=dict, description="Maturity scores per capability.")
