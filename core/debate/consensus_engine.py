...
class ConsensusEvent(BaseModel):
    ...
    event_type: str = Field(..., description="Category: CONSENSUS_INITIATED, ROLE_EVALUATION_START, ROLE_EVALUATION_COMPLETE, REJECTION_TRIGGERED, DISPUTE_RAISED, DISPUTE_RESOLVED, CONSENSUS_COMPLETED")
...
class EngineeringDecisionRecord(BaseModel):
    ...
    considered_alternatives: List[str] = Field(..., description="List of alternatives considered.")
    ...
    architectural_impact: str = Field(..., description="Impact on the codebase architecture.")
    repository_impact: str = Field(..., description="Files, directories, and dependencies affected.")
...
class ConsensusEngine:
    ...
    def run_consensus(self, task_id: str, request_text: str) -> Dict[str, Any]:
        ...
        self.publish_event(task_id, "CONSENSUS_INITIATED", {"request_text": request_text})

        # 1. Product Builder Agent
        self.publish_event(task_id, "ROLE_EVALUATION_START", {"role": "Product Builder"})
        product_builder = self.registry.create_agent("product_builder", self.memory)
        specs = product_builder.run(request_text)
        self.publish_event(task_id, "ROLE_EVALUATION_COMPLETE", {"role": "Product Builder", "output": specs})
        ...
        self.publish_event(task_id, "ROLE_EVALUATION_COMPLETE", {"role": "Planner", "output": plan})
        ...
        self.publish_event(task_id, "ROLE_EVALUATION_START", {"role": "Architect", "action": "Reviewing Plan"})
        arch_review = architect.run(f"Review the planned project steps:\n{plan}")
        self.publish_event(task_id, "ROLE_EVALUATION_COMPLETE", {"role": "Architect", "action": "Reviewing Plan", "output": arch_review})

        is_plan_rejected = "reject" in arch_review.lower() or "disapprove" in arch_review.lower()
        if is_plan_rejected:
            self.publish_event(task_id, "REJECTION_TRIGGERED", {"source": "Architect", "target": "Planner", "reason": arch_review})
            # Request revision
            plan = planner.run(f"Your plan was rejected by the Architect with comments:\n{arch_review}\n\nPlease revise the plan.")
            ...
            if "reject" in arch_review2.lower() or "disapprove" in arch_review2.lower():
                ...
                self.publish_event(task_id, "DISPUTE_RAISED", {"parties": ["Planner", "Architect"], "dispute": arch_review2})
                plan = tech_lead.run(f"Arbitrate dispute: Planner proposed a plan, Architect rejected it twice. Architect comments:\n{arch_review2}\n\nDecide and output the final plan.")
                self.publish_event(task_id, "DISPUTE_RESOLVED", {"resolution": plan})

        # 4. Repository Engineer scans codebase & flags dependencies
        self.publish_event(task_id, "ROLE_EVALUATION_START", {"role": "Repository Engineer"})
        repository_engineer = self.registry.create_agent("repository_engineer", self.memory)
        repo_analysis = repository_engineer.run(plan)
        self.publish_event(task_id, "ROLE_EVALUATION_COMPLETE", {"role": "Repository Engineer", "output": repo_analysis})

        if "conflict" in repo_analysis.lower() or "circular dependency" in repo_analysis.lower():
            self.publish_event(task_id, "DISPUTE_RAISED", {"parties": ["Repository Engineer"], "conflict": repo_analysis})
            plan = tech_lead.run(f"Resolve repository conflicts found in codebase:\n{repo_analysis}\n\nAdjust the plan and output revised plan.")
            self.publish_event(task_id, "DISPUTE_RESOLVED", {"resolution": plan})
        ...
        self.publish_event(task_id, "ROLE_EVALUATION_COMPLETE", {"role": "Researcher", "output": research_findings})

        if "contradiction" in research_findings.lower() or "contradictory" in research_findings.lower():
            self.publish_event(task_id, "DISPUTE_RAISED", {"parties": ["Researcher"], "contradiction": research_findings})
            arbitrated_doc = tech_lead.run(f"Arbitrate documentation contradictions found by Researcher:\n{research_findings}\n\nProvide the standard style or rule to follow.")
            self.publish_event(task_id, "DISPUTE_RESOLVED", {"resolution": arbitrated_doc})
        ...
        self.publish_event(task_id, "ROLE_EVALUATION_COMPLETE", {"role": "Developer", "output": code_proposals})
        ...
        self.publish_event(task_id, "ROLE_EVALUATION_COMPLETE", {"role": "Reviewer", "output": review_comments})

        is_code_rejected = "reject" in review_comments.lower() or "objection" in review_comments.lower() or "fail" in review_comments.lower()
        if is_code_rejected:
            self.publish_event(task_id, "REJECTION_TRIGGERED", {"source": "Reviewer", "target": "Developer", "reason": review_comments})
            # Revise code
            code_proposals = developer.run(f"Your code changes were rejected by the Reviewer. Comments:\n{review_comments}\n\nPlease revise the code.")
            review_comments = reviewer.run(code_proposals)
            
            if "reject" in review_comments.lower() or "objection" in review_comments.lower() or "fail" in review_comments.lower():
                self.publish_event(task_id, "DISPUTE_RAISED", {"parties": ["Developer", "Reviewer"], "dispute": review_comments})
                code_proposals = tech_lead.run(f"Arbitrate Developer-Reviewer dispute. Reviewer rejected the Developer's code twice. Comments:\n{review_comments}\n\nResolve the issue and output the final code changes.")
                self.publish_event(task_id, "DISPUTE_RESOLVED", {"resolution": code_proposals})

        # 8. Architect reviews final code
        self.publish_event(task_id, "ROLE_EVALUATION_START", {"role": "Architect", "action": "Reviewing Code"})
        final_arch_review = architect.run(f"Validate the final code against system architectural standards:\n{code_proposals}")
        self.publish_event(task_id, "ROLE_EVALUATION_COMPLETE", {"role": "Architect", "action": "Reviewing Code", "output": final_arch_review})

        # 9. Tech Lead Approval Sign-off
        self.publish_event(task_id, "ROLE_EVALUATION_START", {"role": "Tech Lead", "action": "Final Approval"})
        tl_verdict = tech_lead.run(f"Render final consensus approval for proposed changes:\n{code_proposals}")
        self.publish_event(task_id, "ROLE_EVALUATION_COMPLETE", {"role": "Tech Lead", "action": "Final Approval", "output": tl_verdict})
        ...
        try:
            ...
            self.publish_event(task_id, "ROLE_EVALUATION_START", {"role": "Human Approval", "action": "Verify"})
            self.publish_event(task_id, "ROLE_EVALUATION_COMPLETE", {"role": "Human Approval", "action": "Verify", "output": "Approved by Human Gate Check"})
        except Exception:
            pass
    ...
    def generate_edr(self, task_id: str, request: str, specs: str, plan: str, code: str) -> EngineeringDecisionRecord:
        """
        ...
        try:
            ...
            logger.warning(f"Structured EDR generation failed, falling back to dummy EDR: {e}")
            return EngineeringDecisionRecord(
                problem_statement=request[:100],
                considered_alternatives=["Use standard sequential flow", "Use manual code edits"],
                ...
                lessons_learned=["Enforce strict token-overlap and architectural reviews early."]
            )
    ...
    def publish_event(self, task_id: str, event_type: str, details: Dict[str, Any]) -> None:
        """
