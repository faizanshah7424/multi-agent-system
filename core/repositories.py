...
from core.database import Task, TaskLog, AgentMessage, MemoryEntry, WorkflowExecution, WorkerHeartbeat
...
class TaskRepository:
    ...
    def create_task(self, task_id: str, payload: dict, user_id: Optional[str] = None, task_type: str = "workflow") -> Task:
    ...
    def add_log(self, task_id: str, source: str, message: str, level: str = "INFO") -> TaskLog:
        log = TaskLog(
            task_id=task_id,
            source=source,
            message=message,
    ...
    def get_logs(self, task_id: str) -> List[TaskLog]:
        return self.session.query(TaskLog).filter(TaskLog.task_id == task_id).order_by(TaskLog.timestamp.asc()).all()

    def add_message(self, task_id: str, role: str, agent_name: str, content: str) -> AgentMessage:
        msg = AgentMessage(
            task_id=task_id,
            role=role,
    ...
    def get_messages(self, task_id: str) -> List[AgentMessage]:
        return self.session.query(AgentMessage).filter(AgentMessage.task_id == task_id).order_by(AgentMessage.timestamp.asc()).all()
    ...
    def recover_stale_tasks(self, max_age_seconds: int = 15) -> List[str]:
        ...
        cutoff_time = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(seconds=max_age_seconds)
        
        stale_workers = self.session.query(WorkerHeartbeat).filter(
        ...
        for worker in stale_workers:
            ...
            tasks_to_recover = self.session.query(Task).filter(
                Task.claimed_by == worker.worker_id,
                Task.status == "RUNNING"
            ).all()
...
class MemoryRepository:
    ...
    def get_entries(self, session_id: str) -> List[MemoryEntry]:
        return self.session.query(MemoryEntry).filter(MemoryEntry.session_id == session_id).order_by(MemoryEntry.timestamp.desc()).all()

    def clear_entries(self, session_id: str) -> None:
        self.session.query(MemoryEntry).filter(MemoryEntry.session_id == session_id).delete()
        self.session.flush()
...
class WorkflowRepository:
    ...
    def update_step(
        ...
        step = self.session.query(WorkflowExecution).filter(
    ...
    def get_steps(self, task_id: str) -> List[WorkflowExecution]:
        return self.session.query(WorkflowExecution).filter(
            WorkflowExecution.task_id == task_id
        ).order_by(WorkflowExecution.step_id.asc()).all()
...
class WorkerRepository:
    ...
    def register_worker(self, worker_id: str, hostname: str, pid: int) -> WorkerHeartbeat:
        worker = self.session.query(WorkerHeartbeat).filter(WorkerHeartbeat.worker_id == worker_id).first()
        if not worker:
    ...
    def update_heartbeat(self, worker_id: str, status: str, active_task_id: Optional[str] = None) -> Optional[WorkerHeartbeat]:
        worker = self.session.query(WorkerHeartbeat).filter(WorkerHeartbeat.worker_id == worker_id).first()
        if worker:
            worker.last_seen = datetime.now(timezone.utc).replace(tzinfo=None)
    ...
    def get_active_workers(self, max_age_seconds: int = 15) -> List[WorkerHeartbeat]:
        from datetime import timedelta
        cutoff_time = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(seconds=max_age_seconds)
        return self.session.query(WorkerHeartbeat).filter(
            WorkerHeartbeat.last_seen >= cutoff_time,
            WorkerHeartbeat.status != "SHUTDOWN"
        ).all()
    ...
    def remove_worker(self, worker_id: str) -> None:
        self.session.query(WorkerHeartbeat).filter(WorkerHeartbeat.worker_id == worker_id).delete()
        self.session.flush()
