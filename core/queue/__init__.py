...
class TaskModel(BaseModel):
    ...
    task_id: str = Field(
        ..., description="Unique task identifier, also used as memory session ID."
    )
    user_id: Optional[str] = Field(default=None, description="Optional user ID owner.")
    task_type: str = Field(
        default="workflow", description="Type classification of the task."
    )
    payload: Dict[str, Any] = Field(
        ..., description="Prompt and variables payload required for agent execution."
    )
    status: TaskStatus = Field(
        default=TaskStatus.PENDING, description="Current task execution status."
    )
...
class Worker(threading.Thread):
    ...
    def __init__(
        self, worker_id: int, task_queue: TaskQueue, manager: "TaskManager"
    ) -> None:
    ...
    def run(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            logger.info(
                f"Worker {self.worker_id} pulled Task '{task_id}' for execution."
            )
            try:
                self.manager.execute_task(task_id, self.stop_event)
            except Exception as e:
                logger.error(
                    f"Worker {self.worker_id} crashed executing task {task_id}: {str(e)}",
                    exc_info=True,
                )
...
class WorkerPool:
    ...
    def __init__(
        self, size: int, task_queue: TaskQueue, manager: "TaskManager"
    ) -> None:
        self.size = size
        self.task_queue = task_queue
        self.manager = manager
    ...
    def start(self) -> None:
        ...
        with self._lock:
            ...
            for i in range(self.size):
                w = Worker(
                    worker_id=i + 1, task_queue=self.task_queue, manager=self.manager
                )
                w.daemon = True
                w.start()
...
class TaskManager:
    ...
    def create_task(
        self, task_id: str, payload: Dict[str, Any], user_id: Optional[str] = None
    ) -> TaskModel:
        """
        ...
        with get_db_session() as session:
            ...
            if not db_task:
                raise AgentNotFoundError(
                    f"Task ID '{task_id}' was not found in the database."
                )
        ...
        with get_db_session() as session:
            ...
            if not db_task:
                db_task = repo.create_task(
                    task_id=task.task_id, payload=task.payload, task_type=task.task_type
                )
            ...
            db_task.started_at = (
                datetime.fromisoformat(task.started_at) if task.started_at else None
            )
    ...
    def recover_tasks(self) -> int:
        ...
        with get_db_session() as session:
            ...
            for task in db_tasks:
                if task.status in ("PENDING", "QUEUED", "RUNNING", "RETRYING"):
                    logger.info(
                        f"Recovering unfinished task '{task.task_id}' (Status was '{task.status}'). Re-queuing..."
                    )
                    task.status = "QUEUED"
        ...
        if recovered_count > 0:
            logger.info(
                f"Task recovery cycle complete. Re-queued {recovered_count} tasks."
            )
    ...
    def execute_task(self, task_id: str, stop_event: threading.Event) -> None:
        ...
        try:
            ...
            if task.status == TaskStatus.CANCELLED:
                logger.info(
                    f"Task {task_id} completed run, but was marked CANCELLED in memory."
                )
                return
    ...
    def handle_task_failure(self, task: TaskModel, error_message: str) -> None:
        ...
        is_retryable = any(
            ...
            for kw in ["429", "quota", "demand", "timeout", "exhausted", "503"]
        )
        ...
        if is_retryable and task.retry_count < max_retries:
            ...
            logger.warning(
                f"Task '{task.task_id}' failed (Attempt {task.retry_count}). Retrying in {backoff_delay}s..."
            )
                ...
                try:
                    ...
                    with get_db_session() as session:
                        ...
                        if db_task and db_task.status not in (
                            ...
                            repo.save_task(db_task)
                            logger.info(
                                f"Re-queued retried task '{task.task_id}' in SQLite."
                            )
                    self.task_queue.put(task.task_id)
                except Exception as e:
                    logger.error(
                        f"Failed to re-queue retried task '{task.task_id}': {e}"
                    )

            t = threading.Thread(
                target=schedule_retry, name=f"RetryTimer-{task.task_id}"
            )
            t.daemon = True
            t.start()
        else:
