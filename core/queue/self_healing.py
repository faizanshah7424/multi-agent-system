...
class ParsedFailure(BaseModel):
    ...
    category: str = Field(
        ...,
        description="Failure category (e.g. SYNTAX_ERROR, IMPORT_ERROR, TYPE_ERROR, LINT_ERROR, UNIT_TEST_FAILURE, MISSING_DEPENDENCY)",
    )
    file_path: Optional[str] = Field(
        default=None, description="Path to the file containing the error."
    )
    line_number: Optional[int] = Field(
        default=None, description="Line number of the error."
    )
    column_number: Optional[int] = Field(
        default=None, description="Column number of the error."
    )
...
class SelfHealingEngine(ISelfHealingEngine):
    ...
    def repair_failure(
        ...
        if not failure:
            logger.warning(
                "Log parsing yielded no recognizable failure pattern. Aborting repair."
            )
            return False
        ...
        if failure.file_path and (
            "test_" in failure.file_path or "tests/" in failure.file_path
        ):
            logger.error(
                f"Safety Gate Violation: Self-healing blocked editing test file: {failure.file_path}"
            )
            return False
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            agent_success = agent_executor.execute_step(
                task_id, repair_step, workspace_path, sandbox
            )
        ...
        logger.error(
            f"Self-healing failed to repair step {step.step_id} after {max_attempts} attempts."
        )
        return False
    ...
    def classify_failure(self, error_log: str) -> Optional[ParsedFailure]:
        ...
        tb_match = re.search(
            r'File "([^"]+)", line (\d+)(?:, in (\w+))?\n\s*(.*?)\n(\w+Error|SyntaxError|NameError|ImportError|TypeError|ValueError): (.*)',
            error_log,
            re.DOTALL,
        )
        if tb_match:
        ...
        tsc_match = re.search(
            r"([^\s:]+\.ts)\((\d+),(\d+)\): error (TS\d+): (.*)", error_log
        )
        if tsc_match:
        ...
        eslint_match = re.search(
            r"([^\s:]+\.js): line (\d+), col (\d+), Error - (.*)", error_log
        )
        if eslint_match:
        ...
        if "syntax" in error_log.lower():
            return ParsedFailure(
                ...
                probable_root_cause="Unspecified syntax error.",
            )
        if "test" in error_log.lower():
            return ParsedFailure(
                category="UNIT_TEST_FAILURE",
                message="Test run failed.",
                probable_root_cause="Unit test assertion crash.",
            )
    ...
    def validate_workspace(self, sandbox: ISandbox) -> bool:
        ...
        if res_lint.exit_code != 0:
            logger.warning(
                f"Validation failed: Linter Ruff found errors: {res_lint.stderr}"
            )
            return False
        ...
        if res_test.exit_code != 0:
            logger.warning(
                f"Validation failed: pytest unit tests failed: {res_test.stderr}"
            )
            return False
