...
class HITLOrchestrator:
    ...
    def register_plan(self, task_id: str, plan: PlanDAG) -> None:
        ...
        with get_db_session() as session:
            record = (
                session.query(DBTaskPlan).filter(DBTaskPlan.task_id == task_id).first()
            )
            if not record:
    ...
    def request_approval(
        self, task_id: str, step: PlanStep, approval_type: str
    ) -> None:
        """
        ...
        try:
            ...
            task.status = (
                TaskStatus.PENDING
            )  # Set back to queue pending so it can be resumed
    ...
    def approve_step(self, task_id: str, step_id: int) -> bool:
        ...
        try:
            ...
            broker.publish(
                "hitl_alerts",
                {"task_id": task_id, "step_id": step_id, "event": "approved"},
            )
        except Exception:
            pass
