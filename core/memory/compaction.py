...
class MemoryCompactionManager:
    ...
    def compact_log(self, verbose_log: str) -> str:
        ...
        for line in lines:
            ...
            if (line_str.startswith("File ") or 
                line_str.startswith("E ") or 
                "error" in line_str.lower() or 
                "failure" in line_str.lower() or
                "warning" in line_str.lower() or
                re.search(r'\.py:\d+:\d+:', line_str) or
                re.search(r'\.ts\(\d+,\d+\):', line_str)):
        ...
        compacted = " | ".join(important_lines[:5])  # Cap to top 5 lines for prompt efficiency
        logger.info(f"Compacted log length from {len(verbose_log)} to {len(compacted)} chars.")
        return compacted
