...
class ILanguageParser(Protocol):
    ...
    """
    def parse(self, file_path: Path, content: str) -> Tuple[List[SymbolDefinition], List[str]]:
        """
...
class JavaScriptLanguageParser:
    ...
    def parse(self, file_path: Path, content: str) -> Tuple[List[SymbolDefinition], List[str]]:
        ...
        func_regex = re.compile(r"\bfunction\s+(\w+)\b|\b(?:const|let|var)\s+(\w+)\s*=\s*(?:\([^)]*\)|[\w$]+)\s*=>")
...
class ASTParser:
    ...
    def _resolve_python_imports(self, current_file: Path, imports: List[str]) -> List[str]:
        ...
        for imp in imports:
            ...
            if py_file.exists():
                resolved.add(str(py_file.relative_to(self.repo_path)).replace("\\", "/"))
            elif init_file.exists():
                resolved.add(str(init_file.relative_to(self.repo_path)).replace("\\", "/"))
            ...
            # 2. Relative import check (relative to current file's folder)
                ...
                if py_rel.exists():
                    resolved.add(str(py_rel.relative_to(self.repo_path)).replace("\\", "/"))
                elif init_rel.exists():
                    resolved.add(str(init_rel.relative_to(self.repo_path)).replace("\\", "/"))
        ...
        for imp in imports:
            ...
            if imp.startswith("."):
                ...
                for ext in (".ts", ".tsx", ".js", ".jsx"):
                    ...
                    if f_path.exists():
                        resolved.add(str(f_path.relative_to(self.repo_path)).replace("\\", "/"))
                        break
                ...
                if target_path.exists() and target_path.is_dir():
                    for ext in (".ts", ".tsx", ".js", ".jsx"):
                        ...
                        if index_f.exists():
                            resolved.add(str(index_f.relative_to(self.repo_path)).replace("\\", "/"))
                            break
