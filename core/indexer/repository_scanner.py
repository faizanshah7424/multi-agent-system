...
class MetadataDetector:
    ...
    def detect_technologies(self, scan_result: Dict[str, Any]) -> List[str]:
        ...
        # 1. Next.js
        if (
            "next.config.js" in configs
            or "next.config.ts" in configs
            or "next" in package_content
        ):
        ...
        # 3. TypeScript
        if "tsconfig.json" in configs or any(
        ...
        # 5. Tailwind CSS
        if (
            "tailwind.config.js" in configs
            or "tailwind.config.ts" in configs
            or "tailwindcss" in package_content
        ):
            detected.add("Tailwind CSS")

        # 6. Prisma
        if (
            "schema.prisma" in configs
            or "prisma" in package_content
            or any("prisma" in f for f in files)
        ):
            detected.add("Prisma")
        ...
        if pyproject_path.exists():
            try:
                pyproj_content = pyproject_path.read_text(
                    encoding="utf-8", errors="replace"
                ).lower()
            except Exception:
                pass
        ...
        if "sqlite3" in combined_python_deps or any(
            f.endswith(".db") or f.endswith(".sqlite") for f in files
        ):
            detected.add("SQLite")
...
class ConventionDetector:
    ...
    def detect_conventions(self, scan_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if max_votes > len(files) * 0.4:  # If one case covers > 40%
            if max_votes == snake_count:
                casing = "snake_case"
            elif max_votes == kebab_count:
                casing = "kebab-case"
            elif max_votes == pascal_count:
                casing = "PascalCase"
            elif max_votes == camel_count:
                casing = "camelCase"
        ...
        layered_hits = sum(
            1 for f in files if any(k in f.split("/") for k in layered_keys)
        )
        ...
        if any(
            f.endswith(".test.py") or f.endswith(".test.ts") or f.endswith(".test.js")
            for f in files
        ):
