...
class RepositoryReportGenerator:
    ...
    def generate_report(
        ...
        routing_files = [f for f in files if "route" in f or "endpoint" in f or "api/" in f or "urls.py" in f]
        
        # Calculate auth files
        auth_files = [f for f in files if any(k in f.lower() for k in ("auth", "login", "session", "jwt", "apikey"))]
        
        # Calculate database files
        db_files = [f for f in files if any(k in f.lower() for k in ("database", "models", "schema.prisma", "migrations"))]
        ...
        report.append(f"- **Core Programming Languages:** {', '.join(scan_data.get('languages', {}).keys())}")
        report.append(f"- **Registered Code Symbols:** {symbol_count} definitions indexed in Code Graph DB.")
        report.append(f"- **Dependency Relationship Edges:** {edges_count} edges mapped.")
        report.append(f"- **Active Frameworks & Technologies:** {', '.join(tech_stack)}")
        
        # Architecture Overview
        report.append("\n## Architecture Overview")
        report.append(f"The repository follows a **{conventions.get('folder_organization', 'layered')}** directory structure layout.")
        report.append("Detailed layout characteristics:")
        report.append(f"- Folder organization style: `{conventions.get('folder_organization')}`")
        report.append(f"- API Endpoint routing layer: `{conventions.get('api_organization')}`")
        report.append(f"- Persistence model: `{conventions.get('database_pattern')}`")
        report.append(f"- Test suite arrangement: `{conventions.get('testing_structure')}`")
        ...
        # Framework Summary
        report.append("\n## Framework Summary")
        report.append("The repository environment integrates the following framework systems:")
        ...
        if circular_imports:
            report.append(f"- **Warning:** {len(circular_imports)} circular dependency cycles detected in imports graph.")
        else:
        ...
        if auth_files:
            ...
            report.append("- No authentication management components detected in codebase.")

        # Database Summary
        report.append("\n## Database Summary")
        report.append(f"Persistence framework: **{conventions.get('database_pattern')}**")
        if db_files:
        ...
        # Coding Conventions
        report.append("\n## Coding Conventions")
        report.append(f"- **Naming casing convention:** `{conventions.get('naming_convention')}`")
        report.append(f"- **Folder architecture pattern:** `{conventions.get('folder_organization')}`")
        ...
        if "FastAPI" in tech_stack:
            report.append("- Adding router endpoints under `api/routes/` or registers in FastAPI `APIRouter` instances.")
        if "Next.js" in tech_stack:
            report.append("- App router folders under `app/` or pages files under `pages/` directory.")
    ...
    def _determine_entry_points(self, files: List[str]) -> List[str]:
        entries = []
        for f in files:
            if f in ("main.py", "app.py", "wsgi.py", "manage.py", "index.js", "index.ts", "server.js", "app.ts"):
                entries.append(f)
            elif "pages/index" in f or "app/page" in f:
                entries.append(f)
        return entries

    def _calculate_risks(self, files: List[str], circular_imports: List[List[str]], tests: List[str]) -> List[str]:
        risks = []
        if circular_imports:
            risks.append(f"**Circular dependencies:** {len(circular_imports)} cycles could cause import locks at runtime.")
        if not tests:
            risks.append("**Missing Tests:** Codebase contains no active test suite directories or test specs.")
        elif len(tests) < len(files) * 0.05:
            risks.append("**Low Test Coverage:** Low ratio of test files to implementation source modules.")
    ...
    def _suggest_insertions(self, files: List[str], tech_stack: List[str]) -> List[str]:
        suggestions = []
        if "FastAPI" in tech_stack:
            suggestions.append("Add endpoint handlers under api/routes/ and declare them in APIRouter initialization.")
        elif "Django" in tech_stack:
            suggestions.append("Create views in app/views.py and add mappings to app/urls.py.")
        
        if "Next.js" in tech_stack:
            suggestions.append("Create page components inside app/ or pages/ folders.")
            
        return suggestions or ["Add new implementation files under core/ or services/ folder structure."]

    def _suggest_refactorings(self, files: List[str], circular_imports: List[List[str]]) -> List[str]:
        suggestions = []
        if circular_imports:
        ...
        if len(files) > 200:
            suggestions.append("Large repository detected. Consider modularizing into distinct packages.")
        return suggestions or ["No immediate refactoring items suggested for codebase structure."]
