...
class DBSymbol(Base):
    ...
    symbol_type = Column(String(50), nullable=False, index=True)  # class, function, method, module, decorator
...
class DBCodeEdge(Base):
    ...
    relation_type = Column(String(50), nullable=False, index=True)  # import, dependency, parent, child
...
class CodeGraphDB:
    ...
    def insert_edges(self, source_file: str, target_file: str, relation_type: str) -> None:
    ...
    def query_edges_by_source(self, source_file: str) -> List[str]:
        ...
        with get_db_session() as session:
            edges = session.query(DBCodeEdge).filter(
                DBCodeEdge.source_file == source_file
            ).all()
    ...
    def query_edges_by_target(self, target_file: str) -> List[str]:
        ...
        with get_db_session() as session:
            edges = session.query(DBCodeEdge).filter(
                DBCodeEdge.target_file == target_file
            ).all()
