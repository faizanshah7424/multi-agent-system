...
class TaskManager:
    ...
    def recover_tasks(self) -> int:
        ...
        Returns:
            The count of recovered and re-queued tasks.
        """
        from core.database import get_db_session
        from core.repositories import TaskRepository
        ...
        with get_db_session() as session:
            ...
            for task in db_tasks:
                # If the task was left in intermediate states, re-enqueue it
    ...
    def execute_task(self, task_id: str, stop_event: threading.Event) -> None:
        ...
        # Configure logging correlation context for the executing thread
        ...
        try:
            # Load inputs from payload
            prompt = task.payload.get("task")
            if not prompt:
                raise ValueError("Payload missing required 'task' prompt key.")

            # Import ManagerAgent locally to avoid recursive loading loops
            from agents.manager import ManagerAgent
            
            # Instantiate ManagerAgent using the task_id as its memory session ID
            # This links Task Queue tracking directly with SharedMemory persistence
            manager = ManagerAgent(session_id=task_id)
            
            # Execute workflow
            manager.execute(prompt)
            
            # Validate status outcome from memory state
            final_memory_status = manager.memory.state.status
            
            # Reload task state (checks if cancelled mid-run)
            task = self.get_task(task_id)
            if task.status == TaskStatus.CANCELLED:
            ...
            if final_memory_status == "completed":
                ...
                # Find last workflow error in memory logs
    ...
    def handle_task_failure(self, task: TaskModel, error_message: str) -> None:
        ...
        Handles API 429 quota exhaustion and timeout errors gracefully.
        """
        max_retries = 3
        # Check if error message indicates transient errors like API quota or demands
        is_retryable = any(kw in error_message.lower() for kw in ["429", "quota", "demand", "timeout", "exhausted", "503"])
        
        # Reload task in case status changed concurrently
        ...
        if is_retryable and task.retry_count < max_retries:
            ...
            # Run delayed re-queue on separate daemon thread to avoid blocking worker pools
            def schedule_retry():
                time.sleep(backoff_delay)
                # Verify that task was not cancelled during sleep
                try:
                    from core.database import get_db_session
                    from core.repositories import TaskRepository
                    with get_db_session() as session:
...
# Global Queue instantiations
task_queue = TaskQueue()
task_manager = TaskManager(task_queue)
# Worker pool sized to 2 by default to prevent free-tier parallel API quota exhaustion
