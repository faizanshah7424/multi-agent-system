...
class WorkerRuntime:
    def __init__(
        ...
        self.worker_id = (
            worker_id or f"worker_{self.hostname}_{self.pid}_{uuid.uuid4().hex[:4]}"
        )
    ...
    def start(self):
        logger.info(
            f"Starting worker {self.worker_id} on {self.hostname} (PID: {self.pid}) with concurrency={self.concurrency}"
        )
        ...
        try:
            ...
            logger.warning(
                "Signal handlers could not be registered (not in main thread). Skipping signal setup."
            )
            ...
            try:
                ...
                if now - last_recovery_check > 10.0:
                    with get_db_session() as session:
                        ...
                        if recovered:
                            logger.info(
                                f"Recovered stale tasks from crashed workers: {recovered}"
                            )
                ...
                if now - last_notification_check > 5.0:
                    try:
                        ...
                        if processed > 0:
                            logger.info(
                                f"Notification queue processor: sent/updated {processed} items."
                            )
                ...
                if task_id:
                    logger.info(
                        f"Claimed task '{task_id}'. Starting execution thread..."
                    )
    ...
    def _execute_task_wrapper(self, task_id: str):
        ...
        with get_db_session() as session:
            ...
            if not db_task:
                logger.error(
                    f"Task '{task_id}' not found in SQLite database. Aborting execution."
                )
                return
            if db_task.claimed_by != self.worker_id:
                logger.error(
                    f"Task '{task_id}' ownership mismatch! Expected worker_id='{self.worker_id}', found claimed_by='{db_task.claimed_by}'. Aborting execution."
                )
                return
            if db_task.status != "RUNNING":
                logger.error(
                    f"Task '{task_id}' status mismatch! Expected status='RUNNING', found status='{db_task.status}'. Aborting execution."
                )
                return
            ...
            if not isinstance(payload, dict):
                logger.error(
                    f"Task '{task_id}' payload is not a dictionary. Failing task."
                )
                self._mark_task_failed(
                    task_id, "Invalid task payload structure: Must be a JSON object."
                )
                return
            ...
            if (
                ...
                or len(task_prompt.strip()) == 0
            ):
                logger.error(
                    f"Task '{task_id}' payload missing 'task' prompt or has invalid format. Failing task."
                )
                self._mark_task_failed(
                    task_id,
                    "Invalid task payload structure: 'task' prompt string must be present and non-empty.",
                )
                return
            ...
            if user_id is not None:
                ...
                if (
                    ...
                    logger.error(
                        f"Task '{task_id}' has invalid owner user_id format. Failing task."
                    )
        ...
        try:
            ...
            logger.error(
                f"Unhandled error in task execution '{task_id}': {e}", exc_info=True
            )
        finally:
            with self.running_tasks_lock:
    ...
    def _heartbeat_loop(self):
        while not self.shutdown_event.is_set():
            try:
                ...
                logger.error(
                    f"Error updating heartbeat for worker {self.worker_id}: {e}"
                )
    ...
    def _wait_for_shutdown(self):
        ...
        while True:
            ...
            if elapsed >= timeout:
                logger.warning(
                    f"Shutdown timeout reached. Requeuing unfinished tasks: {active_tasks}"
                )
                ...
                with get_db_session() as session:
                    ...
                    for task_id in active_tasks:
                        db_task = task_repo.get_task(task_id)
                        if db_task and db_task.status not in (
                            "COMPLETED",
                            "FAILED",
                            "CANCELLED",
                        ):
            ...
            logger.info(
                f"Still waiting for tasks to finish: {active_tasks} ({timeout - elapsed:.1f}s remaining)..."
            )
            time.sleep(1.0)
...
def main():
    ...
    parser.add_argument(
        "--worker-id", type=str, default=None, help="Unique worker identifier"
    )
    parser.add_argument(
        "--concurrency",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Interval in seconds to poll SQLite queue",
    )
