...
class WorkflowEngine:
    ...
    def __init__(self, memory: SharedMemory):
        ...
        for metadata in self.registry.list_agents():
            try:
                self.agents[metadata.name] = self.registry.create_agent(metadata.name, self.memory)
            except Exception as e:
                logger.error(f"Failed to dynamically instantiate registered agent '{metadata.name}': {str(e)}")
    ...
    def validate_plan_graph(self, plan: PlannerPlan) -> List[str]:
        ...
        for step in plan.steps:
            ...
            if step.assigned_agent not in registered_agents:
                errors.append(f"Step {step.step_id}: Assigned agent '{step.assigned_agent}' is not registered in the system registry.")
            
            # Check dependencies
            for dep in step.dependencies:
                if dep not in step_map:
                    errors.append(f"Step {step.step_id}: Dependency {dep} is not a valid step ID in the plan.")
        ...
        if not errors:
            ...
            for step_id in step_ids:
                if visited.get(step_id, 0) == 0:
                    if dfs(step_id):
                        errors.append("The workflow plan contains dependency cycles (not a valid DAG).")
                        break
    ...
    def execute_workflow(self, task: str) -> Dict[str, Any]:
        ...
        set_correlation_context(task_id=session_id, workflow_id=session_id, agent_name="system")
        
        self.memory.update_status("running")
        self.memory.add_log("workflow", f"Starting workflow execution for task: '{task}'")
        
        # Step 1: Planning
        set_correlation_context(task_id=session_id, workflow_id=session_id, agent_name="planner")
        ...
        try:
            ...
            metrics_collector.record_workflow_run(time.time() - start_time, success=False)
        ...
        try:
            ...
            if validation_errors:
                err_msg = f"Workflow plan validation failed with {len(validation_errors)} error(s):\n" + "\n".join(f"- {e}" for e in validation_errors)
        ...
        self.memory.add_log("workflow", f"Plan generated: {plan_data.get('project_title', 'System Plan')}. Total steps: {len(steps)}")
        ...
        for step_dict in serialized_steps:
            ...
            set_correlation_context(task_id=session_id, workflow_id=session_id, agent_name=agent_name)
            
            self.memory.add_log("workflow", f"Beginning Step {step_id}: '{step_name}' -> Assigned to {agent_name}")
            self._update_step(step_id, status="running", started_at=datetime.now(timezone.utc).replace(tzinfo=None).isoformat())
            
            # Select the assigned agent
            agent = self.agents.get(agent_name)
            if not agent:
                logger.warning(f"Agent '{agent_name}' not found. Defaulting to Tool Agent.")
            ...
            try:
                ...
                self.memory.add_log("workflow", f"Step {step_id} completed successfully.")
