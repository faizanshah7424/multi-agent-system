...
class TestWorkerSystem(unittest.TestCase):
    ...
    def test_worker_registration_and_heartbeat(self):
        ...
        with get_db_session() as session:
            ...
            worker_db = (
                ...
                .filter_by(worker_id="test_worker_1")
                .first()
            )
    ...
    def test_stale_worker_recovery(self):
        ...
        with get_db_session() as session:
            ...
            worker_db = (
                session.query(WorkerHeartbeat)
                .filter_by(worker_id="stale_worker")
                .first()
            )
            worker_db.last_seen = datetime.now(timezone.utc).replace(
                tzinfo=None
    ...
    def test_graceful_shutdown(self, mock_manager):
        ...
        with get_db_session() as session:
            ...
            worker_db = (
                session.query(WorkerHeartbeat)
                .filter_by(worker_id="graceful_worker")
                .first()
            )
            self.assertEqual(worker_db.status, "SHUTDOWN")
    ...
    def _run_multi_worker_load_test(
        self, mock_manager, num_workers: int, num_tasks: int
    ):
        ...
        for i in range(num_workers):
            w = WorkerRuntime(
                worker_id=f"concurrent_worker_{i}", concurrency=2, poll_interval=0.05
            )
            workers.append(w)
        ...
        self.assertTrue(
            all_completed,
            f"Timed out waiting for tasks to complete. {incomplete} tasks remaining.",
        )
        ...
        with get_db_session() as session:
            ...
            for task in tasks:
                self.assertEqual(
                    task.status,
                    "COMPLETED",
                    f"Task {task.task_id} status is {task.status}",
                )
                ...
                self.assertEqual(
                    len(claim_logs),
                    1,
                    f"Task {task.task_id} was claimed {len(claim_logs)} times!",
                )
