...
class TestPersistentMemorySystem(unittest.TestCase):
    ...
    def test_memory_recall_and_store_tools(self, mock_embed):
        ...
        res_store = store_tool.run(
            text="FastAPI route validation tips", metadata={"tag": "api"}
        )
    ...
    @patch(
        "core.llm.ask_llm",
        return_value="CONSOLIDATED ABSTRACT LEARNINGS: Use FastAPI with Pydantic for validation.",
    )
    ...
    def test_api_memory_endpoints(self, mock_ask_llm, mock_embed):
        ...
        response = self.client.get(
            "/memory/search", params={"query": "fastapi deploy", "limit": 1}
        )
        ...
        response = self.client.post(
            "/memory/consolidate", params={"session_id": session_id}
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
