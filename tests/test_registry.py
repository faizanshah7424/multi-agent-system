...
class TestAgentRegistry(unittest.TestCase):
    ...
    def test_duplicate_registration_raises_error(self):
        metadata = AgentMetadata(
            name="duplicate_agent", role="First", description="First desc"
        )
        self.registry.register(metadata, MockBaseAgent)

        metadata_dup = AgentMetadata(
            name="duplicate_agent", role="Second", description="Second desc"
        )
    ...
    def test_decorator_registration(self):
        ...
        self.assertEqual(
            self.registry.get_agent_class("decorator_agent"), DecoratedAgent
        )
    ...
    def test_fresh_instance_creation(self):
        metadata = AgentMetadata(
            name="instance_agent", role="Instantiator", description="Instance test"
        )
        self.registry.register(metadata, MockBaseAgent)
        ...
        agent1 = self.registry.create_agent(
            "instance_agent", memory1, custom_arg="first_run"
        )
        agent2 = self.registry.create_agent(
            "instance_agent", memory2, custom_arg="second_run"
        )
...
class DiscoveredAgent(FakeBase):
    pass
"""
            (agents_pkg / "discovered_agent.py").write_text(
            ...
            self.assertIn(
                "fake_discovered_agent", [a.name for a in self.registry.list_agents()]
            )
            agent_instance = self.registry.create_agent(
                "fake_discovered_agent", MockMemory()
            )
