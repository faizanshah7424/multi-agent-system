...
from core.database import (
    ...
    NotificationRecord, NotificationQueueRecord, RefreshTokenRecord,
    Base, engine, init_db
)
...
class TestNotificationSystem(unittest.TestCase):
    ...
    def test_preferences_flow(self):
        ...
        with get_db_session() as session:
            ...
            # Update preferences
            update_preferences(user_id, {"email_enabled": False, "sms_enabled": True}, session)
    ...
    def test_notification_dispatch_respects_preferences(self):
        ...
        with get_db_session() as session:
            ...
            in_app = session.query(NotificationRecord).filter(NotificationRecord.user_id == user_id).first()
            ...
            email_q = session.query(NotificationQueueRecord).filter(
    ...
    def test_queue_worker_success_and_retry(self):
        ...
        with get_db_session() as session:
            success_item = session.query(NotificationQueueRecord).filter(
                NotificationQueueRecord.title == "Task Started"
            ).first()
            ...
            failed_item = session.query(NotificationQueueRecord).filter(
                NotificationQueueRecord.title == "Connection Error Test"
            ).first()
            ...
            failed_item.next_attempt_at = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(minutes=1)
            session.commit()
    ...
    def test_notification_endpoints(self):
        ...
        signup_res = self.client.post("/auth/signup", json={"email": "client@orbit.ai", "password": "password123"})
        ...
        login_res = self.client.post("/auth/login", json={"email": "client@orbit.ai", "password": "password123"})
        ...
        put_pref = self.client.put("/notifications/preferences", json={"email_enabled": False}, headers=headers)
        ...
        test_trigger = self.client.post("/notifications/test", json={
