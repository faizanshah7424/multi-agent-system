...
from core.workspace.interface import IWorkspaceManager, IWorkspaceSessionManager, FileChange
...
class TestWorkspaceAndSandbox(unittest.TestCase):
    ...
    def test_workspace_transactional_lifecycle(self) -> None:
        ...
        change_add = FileChange(
            ...
            action="add",
            content="Hello from Sprint 4!"
        )
        self.workspace_mgr.stage_changes(self.task_id, [change_add])
        
        target_file = Path(path) / unique_file
        self.assertTrue(target_file.exists())
        self.assertEqual(target_file.read_text(encoding="utf-8"), "Hello from Sprint 4!")
    ...
    def test_workspace_session_manager(self) -> None:
        ...
        with get_db_session() as session:
            record = session.query(DBSessionState).filter(DBSessionState.task_id == self.task_id).first()
    ...
    def test_local_process_sandbox_execution(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            res_timeout = sandbox.execute(["python", "-c", "import time; time.sleep(10)"], timeout=1.0)
            ...
            self.assertEqual(sandbox_file.read_text(encoding="utf-8"), "content to sandbox")
            ...
            self.assertEqual(local_dest.read_text(encoding="utf-8"), "content to sandbox")
