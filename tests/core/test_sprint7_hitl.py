...
class TestHITLAndBroker(unittest.TestCase):
    ...
    def test_hitl_orchestration_pause_and_resume(self) -> None:
        ...
        s1 = PlanStep(step_id=1, dependencies=[], assigned_agent="developer", description="Step 1 description")
        s2 = PlanStep(step_id=2, dependencies=[1], assigned_agent="reviewer", description="Merge task branch back to main")
        s3 = PlanStep(step_id=3, dependencies=[2], assigned_agent="developer", description="Step 3 description")
    ...
    def test_hitl_rejection(self) -> None:
        ...
        # Reject Step 2
        self.hitl_orchestrator.reject_step(self.task_id, 2, "Dangerous command check failed.")
