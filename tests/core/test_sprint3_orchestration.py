...
from core.registry.interface import (
    IToolRegistry,
    ISkillRegistry,
    IPromptLibrary,
    ToolDefinition,
...
class TestAIOrchestration(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.test_skill_path = (
            Path(__file__).parent.parent / "resources" / "skills" / "test_skill.md"
        )
    ...
    def test_model_provider_registry(self) -> None:
        ...
        ModelProviderRegistry.register(
            "openai", openai, ModelCapabilities(model_name="gpt-4o")
        )
        ModelProviderRegistry.register(
            "anthropic", anthropic, ModelCapabilities(model_name="claude-3-5-sonnet")
        )
    ...
    def test_session_serialization(self) -> None:
        ...
        conv = Conversation(
            ...
            total_cost=Cost(
                input_cost_usd=0.0001, output_cost_usd=0.0002, total_cost_usd=0.0003
            ),
        )
