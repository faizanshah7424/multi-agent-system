...
class TestTaskQueueAndWorkerSystem(unittest.TestCase):
    ...
    def test_task_manager_cancellation(self):
        task = task_manager.create_task(
            task_id="task_cancel_test", payload={"task": "Cancel me"}
        )
        self.assertEqual(task.status, TaskStatus.QUEUED)
    ...
    def test_retry_system_transient_failure(self):
        ...
        task_manager.handle_task_failure(
            task, "Gemini API failure: ResourceExhausted 429 rate limit exceeded"
        )
    ...
    def test_worker_execution_flow(self):
        ...
        task = task_manager.create_task(
            task_id="task_exec_test", payload={"task": "Mocked workflow run"}
        )
