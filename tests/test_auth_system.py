...
class TestAuthSystem(unittest.TestCase):
    ...
    def test_signup_login_refresh_flow(self):
        ...
        login_res = self.client.post(
            "/auth/login",
            json={"email": "user@orbit.ai", "password": "securepassword123"},
        )
        ...
        # Test Logout
        logout_res = self.client.post("/auth/logout", json={"refresh_token": refresh})
        ...
        failed_refresh_res = self.client.post(
    ...
    def test_forgot_reset_password(self):
        self.client.post(
            "/auth/signup",
            json={"email": "user2@orbit.ai", "password": "originalpassword"},
        )

        forgot_res = self.client.post(
            "/auth/forgot-password", json={"email": "user2@orbit.ai"}
        )
        ...
        login_res = self.client.post(
            "/auth/login",
            json={"email": "user2@orbit.ai", "password": "newpassword123"},
        )
        self.assertEqual(login_res.status_code, 200)
    ...
    def test_rbac_validation(self):
        ...
        res1 = self.client.get(
            "/auth/admin-only", headers={"Authorization": f"Bearer {dev_token}"}
        )
        self.assertEqual(res1.status_code, 403)

        res2 = self.client.get(
            "/auth/dev-or-admin", headers={"Authorization": f"Bearer {dev_token}"}
        )
