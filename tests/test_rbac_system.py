...
from core.database import get_db_session, User, RoleRecord, PermissionRecord, RolePermissionRecord, AuthAuditLogRecord, RefreshTokenRecord
...
class TestRBACSystem(unittest.TestCase):
    ...
    def test_rbac_endpoint_protection(self):
        ...
        signup_admin = self.client.post("/auth/signup", json={
        ...
        signup_dev = self.client.post("/auth/signup", json={
            "email": "dev@orbit.ai",
            "password": "password123",
            "role": "developer"
        })
        ...
        signup_mem = self.client.post("/auth/signup", json={
            "email": "mem@orbit.ai",
            "password": "password123",
            "role": "member"
        })
        ...
        login_admin = self.client.post("/auth/login", json={"email": "admin@orbit.ai", "password": "password123"})
        jwt_admin = login_admin.json()["access_token"]
        
        login_dev = self.client.post("/auth/login", json={"email": "dev@orbit.ai", "password": "password123"})
        jwt_dev = login_dev.json()["access_token"]
        
        login_mem = self.client.post("/auth/login", json={"email": "mem@orbit.ai", "password": "password123"})
        jwt_mem = login_mem.json()["access_token"]
        
        # 1. Admin checks (should access all)
        res_users_admin = self.client.get("/admin/users", headers={"Authorization": f"Bearer {jwt_admin}"})
        self.assertEqual(res_users_admin.status_code, 200)
        self.assertEqual(len(res_users_admin.json()), 3)
        
        res_audit_admin = self.client.get("/admin/audit-logs", headers={"Authorization": f"Bearer {jwt_admin}"})
        self.assertEqual(res_audit_admin.status_code, 200)
        
        # 2. Developer checks
        # Developer has users:read and audit:read, so this should pass (200)
        res_users_dev = self.client.get("/admin/users", headers={"Authorization": f"Bearer {jwt_dev}"})
        self.assertEqual(res_users_dev.status_code, 200)
        
        res_audit_dev = self.client.get("/admin/audit-logs", headers={"Authorization": f"Bearer {jwt_dev}"})
        self.assertEqual(res_audit_dev.status_code, 200)
        
        # Developer does NOT have roles:read, so this should fail (403)
        res_roles_dev = self.client.get("/admin/roles", headers={"Authorization": f"Bearer {jwt_dev}"})
        self.assertEqual(res_roles_dev.status_code, 403)
        
        # 3. Member checks
        # Member only has users:read, so audit:read should fail (403)
        res_audit_mem = self.client.get("/admin/audit-logs", headers={"Authorization": f"Bearer {jwt_mem}"})
        self.assertEqual(res_audit_mem.status_code, 403)

    def test_user_management_and_audit_logging(self):
        # Create admin and developer
        self.client.post("/auth/signup", json={"email": "admin2@orbit.ai", "password": "password123", "role": "admin"})
        admin_login = self.client.post("/auth/login", json={"email": "admin2@orbit.ai", "password": "password123"})
        jwt_admin = admin_login.json()["access_token"]
        
        signup_dev = self.client.post("/auth/signup", json={"email": "dev2@orbit.ai", "password": "password123", "role": "developer"})
        ...
        audit_res = self.client.get("/admin/audit-logs", headers={"Authorization": f"Bearer {jwt_admin}"})
    ...
    def test_role_permissions_updating(self):
        # Create admin
        self.client.post("/auth/signup", json={"email": "admin3@orbit.ai", "password": "password123", "role": "admin"})
        admin_login = self.client.post("/auth/login", json={"email": "admin3@orbit.ai", "password": "password123"})
        jwt_admin = admin_login.json()["access_token"]
        
        # Create developer for testing access later
        self.client.post("/auth/signup", json={"email": "dev3@orbit.ai", "password": "password123", "role": "developer"})
        dev_login = self.client.post("/auth/login", json={"email": "dev3@orbit.ai", "password": "password123"})
        jwt_dev = dev_login.json()["access_token"]
        
        # Developer has tasks:create by default, let's verify dev has access to users list
        pre_check = self.client.get("/admin/users", headers={"Authorization": f"Bearer {jwt_dev}"})
        self.assertEqual(pre_check.status_code, 200)
        
        # Get roles to find developer role id
        roles_res = self.client.get("/admin/roles", headers={"Authorization": f"Bearer {jwt_admin}"})
        ...
        post_check = self.client.get("/admin/users", headers={"Authorization": f"Bearer {jwt_dev}"})
