...
from core.database import (
    Base, engine, get_db_session, init_db,
    Task as DBTask, TaskLog as DBTaskLog, AgentMessage as DBAgentMessage,
    MemoryEntry as DBMemoryEntry, WorkflowExecution as DBWorkflowExecution
)
...
class TestDatabasePersistence(unittest.TestCase):
    ...
    def test_database_transaction_rollback(self):
        ...
        with get_db_session() as session:
            ...
            self.assertIsNone(db_task, "Task should have been rolled back and not exist in DB.")
    ...
    def test_task_repository_operations(self):
        with get_db_session() as session:
            ...
            # Create
            task = repo.create_task("task_repo_1", {"command": "hello"}, user_id="user_123")
            ...
            repo.add_message("task_repo_1", "developer", "researcher", "Here are findings")
    ...
    def test_memory_repository_operations(self):
        with get_db_session() as session:
            ...
            # Add entries
            repo.add_entry("test_index", "Memory text 1", {"tag": "a", "vector": [0.1, 0.2]})
            repo.add_entry("test_index", "Memory text 2", {"tag": "b", "vector": [0.3, 0.4]})
            
            entries = repo.get_entries("test_index")
            self.assertEqual(len(entries), 2)
            self.assertEqual(entries[0].text, "Memory text 2") # sorted by desc timestamp
    ...
    def test_workflow_repository_operations(self):
        ...
        with get_db_session() as session:
            ...
            # Update step status
            wf_repo.update_step("wf_task_1", 1, "completed", result="Plan created successfully", completed_at=datetime.now(timezone.utc).replace(tzinfo=None))
    ...
    def test_migration_layer(self):
        ...
        session_data = {
            ...
            "logs": [
                {"agent": "planner", "message": "Planning steps", "level": "INFO", "timestamp": datetime.now(timezone.utc).replace(tzinfo=None).isoformat()}
            ],
            "messages": [
                {"sender": "planner", "receiver": "developer", "content": "Here is the plan", "timestamp": datetime.now(timezone.utc).replace(tzinfo=None).isoformat()}
            ],
            "data": {
                "workflow_steps": [
                    {
        ...
        with open(memory_dir / "migrated_vector_index.json", "w", encoding="utf-8") as f:
        ...
        with get_db_session() as session:
            ...
            self.assertEqual(migrated_task.payload_json, {"prompt": "Run migration test"})
    ...
    def test_sqlite_concurrency_wal(self):
        ...
        def worker_thread(thread_idx):
            for i in range(20):
                try:
                    with get_db_session() as session:
                        repo = TaskRepository(session)
                        repo.add_log(task_id, f"thread_{thread_idx}", f"log message {i}")
        ...
        self.assertEqual(len(errors), 0, f"Concurrent threads raised SQLite errors: {errors}")
