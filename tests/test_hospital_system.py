...
from core.database import (
    get_db_session, init_db, PatientRecord, DoctorRecord, AppointmentRecord,
...
class TestHospitalSystem(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with get_db_session() as session:
            ...
            if u:
                session.query(RefreshTokenRecord).filter(RefreshTokenRecord.user_id == u.id).delete()
                session.delete(u)
            session.commit()

        # Signup a test doctor/user to use for authenticated endpoints
        signup_res = self.client.post("/auth/signup", json={"email": "nurse@orbit.ai", "password": "password123"})
        ...
        login_res = self.client.post("/auth/login", json={"email": "nurse@orbit.ai", "password": "password123"})
    ...
    def test_patients_endpoints(self):
        # 1. Create Patient
        res = self.client.post("/hospital/patients", json={
            "name": "Jane Doe",
            "email": "jane@doe.com",
            "phone": "555-1234",
    ...
    def test_doctors_endpoints(self):
        # 1. Register Doctor
        res = self.client.post("/hospital/doctors", json={
    ...
    def test_appointments_endpoints(self):
        # Seed Patient & Doctor
        with get_db_session() as session:
            pat = PatientRecord(name="Tim Vance", date_of_birth="1980-01-01", gender="Male")
        ...
        # 1. Book Appointment
        res = self.client.post("/hospital/appointments", json={
            "patient_id": pat_id,
            "doctor_id": doc_id,
        ...
        # 3. Verify in DB
        with get_db_session() as session:
            appt = session.query(AppointmentRecord).filter(AppointmentRecord.id == appt_id).first()
            self.assertEqual(appt.status, "completed")

    def test_billing_endpoints(self):
        # Seed Patient
        with get_db_session() as session:
            pat = PatientRecord(name="Arthur Dent", date_of_birth="1979-05-25", gender="Male")
            session.add(pat)
            session.commit()
            pat_id = pat.id

        # 1. Create Invoice
        res = self.client.post("/hospital/billing", json={
            "patient_id": pat_id,
            "amount": 250.50
        }, headers=self.headers)
        self.assertEqual(res.status_code, 200)
        ...
        pay_res = self.client.put(f"/hospital/billing/{bill_id}/pay", json={
            "payment_method": "Insurance Coverage"
        }, headers=self.headers)
        self.assertEqual(pay_res.status_code, 200)

        # 3. Verify Payment
        with get_db_session() as session:
            bill = session.query(BillingRecord).filter(BillingRecord.id == bill_id).first()
    ...
    def test_pharmacy_dispensary_flow(self):
        ...
        with get_db_session() as session:
            pat = PatientRecord(name="Clark Kent", date_of_birth="1938-04-18", gender="Male")
            doc = DoctorRecord(name="Dr. Jor-El", specialty="Kryptonian biology")
            item = PharmacyItemRecord(name="Kryptonite Inhibitor", dosage="250mg", stock_quantity=10, unit_price=99.00)
        ...
        res = self.client.post("/hospital/pharmacy/prescriptions", json={
            "patient_id": pat_id,
            "doctor_id": doc_id,
        ...
        disp_res = self.client.put(f"/hospital/pharmacy/prescriptions/{pres_id}/dispense", headers=self.headers)
        self.assertEqual(disp_res.status_code, 200)

        # 3. Assert counts
        with get_db_session() as session:
            db_item = session.query(PharmacyItemRecord).filter(PharmacyItemRecord.id == item_id).first()
            self.assertEqual(db_item.stock_quantity, 9)
            
            db_pres = session.query(PrescriptionRecord).filter(PrescriptionRecord.id == pres_id).first()
            self.assertEqual(db_pres.status, "dispensed")

    def test_laboratory_endpoints(self):
        # Seed Patient
        with get_db_session() as session:
            pat = PatientRecord(name="Bruce Wayne", date_of_birth="1939-05-01", gender="Male")
            session.add(pat)
            session.commit()
            pat_id = pat.id

        # 1. Request Lab test
        res = self.client.post("/hospital/lab/tests", json={
            "patient_id": pat_id,
            "test_name": "Toxin analysis"
        }, headers=self.headers)
        self.assertEqual(res.status_code, 200)
        test_id = res.json()["id"]

        # 2. Complete lab test results
        comp_res = self.client.put(f"/hospital/lab/tests/{test_id}/complete", json={
        ...
        with get_db_session() as session:
            db_test = session.query(LabTestRecord).filter(LabTestRecord.id == test_id).first()
    ...
    def test_hospital_dashboard_stats(self):
        ...
        with get_db_session() as session:
            p = PatientRecord(name="Test Patient", date_of_birth="2000-01-01", gender="Other")
            d = DoctorRecord(name="Test Doctor", specialty="General Practice")
            session.add_all([p, d])
            session.commit()

            appt = AppointmentRecord(patient_id=p.id, doctor_id=d.id, appointment_date=datetime.now(), reason="Checkup")
            bill = BillingRecord(patient_id=p.id, amount=100.0, status="unpaid")
            item = PharmacyItemRecord(name="Paracetamol", dosage="500mg", stock_quantity=100, unit_price=0.20)
            pres = PrescriptionRecord(patient_id=p.id, doctor_id=d.id, medication_name="Paracetamol", dosage_instruction="1 tab as needed")
