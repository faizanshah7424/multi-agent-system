            ...
            {successInfo.resetToken && (
              <div className="bg-zinc-950/80 border border-zinc-800 rounded-xl p-4 text-left space-y-3">
                <span className="block text-xs font-semibold text-zinc-500 uppercase tracking-widest">
                  Self-Recovery Console
                </span>
                <p className="text-xs text-zinc-400">
                  Since SMTP services are mocked in SQLite, you can manually trigger your reset below using this token:
                </p>
                <Link
                  href={`/reset-password?token=${successInfo.resetToken}`}
                  className="inline-flex w-full justify-center items-center py-2 px-3 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-lg text-xs transition"
                >
                  Reset Password Now
                </Link>
              </div>
