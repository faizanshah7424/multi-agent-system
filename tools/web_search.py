...
class WebSearchTool(BaseTool):
    ...
    def execute(self, query: str, max_results: int = 5) -> str:
        ...
        try:
            ...
            titles_matches = re.findall(r'<a class="result__a"[^>]*>(.*?)</a>', html_content, re.DOTALL)
            snippets_matches = re.findall(r'<a class="result__snippet"[^>]*>(.*?)</a>', html_content, re.DOTALL)
            
            if not titles_matches:
                # If HTML layout changed or scraping is blocked, return a friendly mock search result warning
                logger.warning("Scraping returned zero search results. Falling back to local index message.")
                return (
            ...
            for i in range(count):
                ...
                title = urllib.parse.unquote(title).replace("&amp;", "&").replace("&quot;", '"').replace("&#x27;", "'")
                snippet = urllib.parse.unquote(snippet).replace("&amp;", "&").replace("&quot;", '"').replace("&#x27;", "'")
