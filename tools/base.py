...
class BaseTool(ABC):
    ...
    name: str = Field(
        ..., description="The name of the tool, used by the model for tool-calling."
    )
...
def validate_safe_path(target_path_str: str) -> Path:
    ...
    if any(
        c in target_path_str
        for c in (chr(i) for i in range(32) if i not in (9, 10, 13))
    ):
    ...
    try:
        ...
        try:
            ...
            raise ValueError(
                f"Access Denied: Path '{target_path_str}' could not be resolved safely."
            )
