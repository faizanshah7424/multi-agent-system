...
def verify_code_safety(code: str) -> None:
    ...
    BANNED_IMPORTS = {
        'subprocess', 'socket', 'ctypes', 'pty', 'webbrowser', 
        'requests', 'urllib', 'httpx', 'ftplib', 'smtplib', 'telnetlib',
        'multiprocessing', 'threading', 'concurrent', 'os', 'sys',
        'importlib', 'builtins', 'shutil', 'pathlib', 'platform',
        'runpy', 'code', 'compileall', 'winreg', 'bdb', 'pdb',
    ...
    BANNED_CALLS = {
        ...
        'getattr', 'setattr', 'hasattr', 'delattr', 'open', 'file'
    }
    
    # Banned os execution functions (fallback check)
    BANNED_OS_FUNCS = {
        'system', 'popen', 'exec', 'execve', 'execl', 'execle', 'execlp', 'execlpe', 
        'execv', 'execvp', 'execvpe', 'spawn', 'spawnl', 'spawnle', 'spawnlp', 'spawnlpe',
        'spawnv', 'spawnvp', 'spawnvpe', 'fork', 'forkpty', 'kill', 'killpg', 'startfile'
    }
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Import):
            for alias in node.names:
                ...
                if name in BANNED_IMPORTS:
                    raise ValueError(f"Import of banned module '{name}' is forbidden for security reasons.")
        ...
        # 2. Check from-imports (e.g., from subprocess import Popen)
            ...
            if node.module:
                ...
                if name in BANNED_IMPORTS:
                    raise ValueError(f"Import from banned module '{name}' is forbidden for security reasons.")
        ...
        # 3. Check function calls
            ...
            if isinstance(node.func, ast.Name):
                if node.func.id in BANNED_CALLS:
                    raise ValueError(f"Calling built-in function '{node.func.id}' is forbidden for security reasons.")
            # Attribute calls (like os.system or sys.modules)
            elif isinstance(node.func, ast.Attribute):
                if isinstance(node.func.value, ast.Name):
                    val_id = node.func.value.id
                    if val_id in BANNED_IMPORTS:
                        raise ValueError(f"Accessing attributes of banned module '{val_id}' is forbidden.")
                if node.func.attr in BANNED_OS_FUNCS or node.func.attr in BANNED_CALLS:
                    raise ValueError(f"Calling dangerous attribute '{node.func.attr}' is forbidden.")
        ...
        # 4. Check Names (prevent reference to banned modules or __builtins__)
            ...
            if node.id.startswith("__") and node.id not in ("__name__", "__main__"):
                raise ValueError(f"Accessing double-underscore identifier '{node.id}' is forbidden.")
            if node.id in BANNED_IMPORTS:
                raise ValueError(f"Usage of banned identifier '{node.id}' is forbidden.")

        # 5. Check Attributes (prevent double-underscore sandbox bypass e.g. __class__, __subclasses__)
        elif isinstance(node, ast.Attribute):
            if node.attr.startswith("__"):
                raise ValueError(f"Accessing double-underscore attribute '{node.attr}' is forbidden.")
            if isinstance(node.value, ast.Name) and node.value.id in BANNED_IMPORTS:
                raise ValueError(f"Accessing attribute '{node.attr}' on banned module '{node.value.id}' is forbidden.")
    ...
    def execute(self, code: str) -> str:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Could not delete temporary script file {temp_file_path}: {str(e)}")
